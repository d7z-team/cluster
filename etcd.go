...
func (s *etcdStore) list(ctx context.Context, scope resourceScope) ([]Unstructured, uint64, error) {
	...
	txnResp, err := s.client.Txn(ctx).Then(
		...
		clientv3.OpGet(s.metaKey("rv"), clientv3.WithLimit(1)),
	).Commit()
	if err != nil {
		return nil, 0, err
	}
	...
	objectResp := txnResp.Responses[0].GetResponseRange()
	metaResp := txnResp.Responses[1].GetResponseRange()
	objects := make([]Unstructured, 0, len(objectResp.Kvs))
	for _, kv := range objectResp.Kvs {
		var obj Unstructured
	...
	if len(metaResp.Kvs) > 0 {
		rv = parseStoredRV(string(metaResp.Kvs[0].Value))
	}
